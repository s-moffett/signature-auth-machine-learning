...
def load():
  ...
  for user in range(1, 41):
    ...
    samples = pd.read_csv('./Input/Task1/U1S1.TXT', delimiter=' ', skiprows=1, names=['x', 'y', 'time', 'button'])
    ...
    for sample in range(2, 41):
      data = pd.read_csv('./Input/Task1/U' + str(user) + 'S' + str(sample) + '.TXT', delimiter=' ', skiprows=1, names=['x', 'y', 'time', 'button'])
